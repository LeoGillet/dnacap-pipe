...
def _list_supported_sequences() -> list[str]:
    ...
    supported_files = [file for file in all_files if file.endswith('.fastq.gz') or file.endswith('.fq.gz')]
    log(f"Input folder contains {len(all_files)} files ({len(supported_files)} supported, {len(all_files) - len(supported_files)} ignored)")
...
def _display_paired_sequences(sequences: dict[tuple[str, str, str]]):
    ...
    """
    for i, (basename, (f, r, ext)) in enumerate(sequences.items()):
        print(f"{i+1}:\t{f}\t<->\t{r}\t({ext})")
